<script lang="ts">
  ...
  import SphereBackdrop from 'demo/utils/SphereBackdrop.svelte';
  import { useButtonState } from 'demo/utils/use-button-state.svelte';
    ...
    <div class="row" class:invert={!options.vertical}>
      <div class="column">
        ...
        <SphereBackdrop glass={options.glass}>
          <NeoTabsRow
            tabs={items}
            bind:active={collapseActive}
            bind:value={collapseContext}
            onchange={onChange}
            ontabfocus={(...args) => {
              console.info('Tab focus', args);
            }}
            ontabhover={(...args) => {
              console.info('Tab hover', args);
            }}
            {skeleton}
            {onclose}
            {onadd}
