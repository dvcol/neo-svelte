...
import Harness from './TestPopConfirm.browser.test.svelte';
...
describe('neoPopConfirm — placement (real layout)', { tags: ['browser'] }, () => {
...
describe('neoPopConfirm — dismiss contract', { tags: ['browser'] }, () => {
  it('escape closes the popover', async () => {
    ...
    await vi.waitFor(() => {
      const tt = getTooltip();
      // Default unmountOnClose=true on tooltip => removed from DOM, OR hidden=true.
      if (!tt) return;
...
describe('neoPopConfirm — visual contract (open + cardinal placements)', { tags: ['browser', 'visual'] }, () => {
  beforeEach(() => {
    quietForVisual();
  });
    ...
    for (const placement of CARDINAL_PLACEMENTS) {
      it(`open at ${placement} (${viewport})`, async () => {
        ...
        await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
          screenshotName('NeoPopConfirm', `open-${placement}`, viewport),
        );
      });
    }
  });
