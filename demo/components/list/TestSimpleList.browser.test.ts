...
import VisualHarness from './TestSimpleList.browser.test.svelte';
...
describe('neoSimpleList — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  beforeEach(() => {
    quietForVisual();
  });

  it('items / loading / empty × tinted / glass / flat matrix (desktop)', { timeout: 30000 }, async () => {
    ...
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoSimpleList', 'matrix', 'desktop'),
    );
  });
});
