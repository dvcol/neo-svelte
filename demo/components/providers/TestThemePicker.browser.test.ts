...
import VisualHarness from './TestThemePicker.browser.test.svelte';
...
describe('neoThemePicker — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  beforeEach(() => {
    quietForVisual();
  });

  it.each(VIEWPORT_NAMES)('default + rounded layout (%s)', async (viewport: ViewportName) => {
    ...
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoThemePicker', 'matrix', viewport),
    );
  });
});
