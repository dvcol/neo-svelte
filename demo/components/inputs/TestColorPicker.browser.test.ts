...
describe('neoColorPicker — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  ...
  it.each(VIEWPORT_NAMES)('empty / filled / disabled / readonly × glass / tinted / sharp / invalid matrix (%s)', async (viewport: ViewportName) => {
    ...
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoColorPicker', 'matrix', viewport),
    );
  });
});
