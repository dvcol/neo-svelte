...
describe('neoTextarea — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  ...
  it('idle / filled / disabled / readonly / invalid / valid × glass / tinted / sharp × pressed / borderless / clearable matrix (desktop)', async () => {
    ...
    await vi.waitFor(() => {
      const textareas = stage.querySelectorAll<HTMLElement>('.neo-textarea');
      expect(textareas.length).toBe(12);
      for (const t of textareas) expect(t.getBoundingClientRect().width).toBeGreaterThan(0);
    });
    await waitForVisualStability(stage);
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoTextarea', 'matrix', 'desktop'),
    );
  });
});
