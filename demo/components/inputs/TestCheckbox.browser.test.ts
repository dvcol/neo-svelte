...
describe('neoCheckbox — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  ...
  it.each(VIEWPORT_NAMES)('checked / indeterminate / disabled × glass / tinted / sharp × valid / invalid / required matrix (%s)', { timeout: 30000 }, async (viewport: ViewportName) => {
    ...
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoCheckbox', 'matrix', viewport),
    );
  });
});
