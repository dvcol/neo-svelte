...
import VisualHarness from './TestDateTime.browser.test.svelte';
...
describe('neoDateTime — visual contract (themed)', { tags: ['browser', 'visual'] }, () => {
  beforeEach(() => {
    quietForVisual();
  });

  it.each(VIEWPORT_NAMES)('date / time / datetime × empty / disabled / readonly × glass / tinted / sharp matrix (%s)', async (viewport: ViewportName) => {
    ...
    await expect.element(page.elementLocator(document.body)).toMatchScreenshot(
      screenshotName('NeoDateTime', 'matrix', viewport),
    );
  });
});
