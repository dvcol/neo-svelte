...
import VisualHarness from './TestAccordion.pseudo.browser.test.svelte';
...
describe('neoAccordion — real CSS pseudo states (themed)', { tags: ['browser', 'visual'] }, () => {
  beforeEach(() => {
    quietForVisual();
  });
  ...
  it('idle baseline grid (desktop)', async () => {
    ...
    await expect.element(page.getByTestId('visual-stage')).toMatchScreenshot(
      screenshotName('NeoAccordion', 'pseudo-idle', 'desktop'),
    );
  });
  ...
  it('hover (desktop)', async () => {
    ...
    render(VisualHarness, { props: {} as never });
    const stage = await vi.waitFor(getStage);
    const acc = await vi.waitFor(() => getTarget());
    await userEvent.hover(acc);
    await waitForVisualStability(stage);
    await expect.element(page.getByTestId('visual-stage')).toMatchScreenshot(
      screenshotName('NeoAccordion', 'pseudo-hover', 'desktop'),
    );
  });
  ...
  it('focus-within — trigger focused (desktop)', async () => {
    ...
    await waitForVisualStability(stage);
    await expect.element(page.getByTestId('visual-stage')).toMatchScreenshot(
      screenshotName('NeoAccordion', 'pseudo-focus-within', 'desktop'),
    );
  });
});
