...
This file is for contributors and AI agents working in this repo. End-user docs live in [README.md](./README.md) and [USAGE.md](./USAGE.md). The component roadmap lives in [ROADMAP.md](./ROADMAP.md).
...
- Per-scope `AGENTS.md` files in `src/lib/<scope>/` are part of the published package — `svelte-package` copies them into `dist/<scope>/` automatically alongside the source code (same path it uses for `.scss` / `.css` assets).

## Documentation map

The repo has a layered doc structure so consumers and contributors load only what they need:

| File                        | Audience     | Published? | Purpose                                                                |
| --------------------------- | ------------ | ---------- | ---------------------------------------------------------------------- |
| `README.md`                 | Everyone     | Yes        | Marketing intro + quick install.                                       |
| `AGENTS.md`                 | AI agents    | Yes        | Tiny router → `USAGE.md` or `CONTRIBUTING.md`.                         |
| `CLAUDE.md`                 | AI agents    | Yes        | Stub deferring to `AGENTS.md`.                                         |
| `USAGE.md`                  | Consumers    | Yes        | Cross-cutting concepts (theming, subpath imports, runes, floating, …). |
| `CONTRIBUTING.md`           | Contributors | **No**     | This file. Test split, ESLint boundary, quality bar.                   |
| `ROADMAP.md`                | Contributors | No         | Component surface — shipped vs. planned.                               |
| `src/lib/<scope>/AGENTS.md` | Consumers    | Yes        | Per-scope guide (import path, components, concepts, gotchas).          |

### Rule: creating or editing a component MUST update `AGENTS.md`

When you add, rename, remove, or meaningfully change the API of a component:

1. **Update the scope's `src/lib/<scope>/AGENTS.md`** — add/remove the component from the _Components_ list, and revise the _Concepts_, _Common pattern_, or _Gotchas_ sections if the change is observable from the outside (new prop, new snippet, new behavior, new gotcha, breaking rename).
2. **Update `USAGE.md`** only if the change introduces a new cross-cutting concept (new mandatory provider, new global token, a new floating-config knob shared by every floating component). Per-component prop tweaks do not belong here.
3. **Update the scope index table in `USAGE.md`** if you add a new top-level scope (or split/merge an existing one).
4. **Update `package.json` `exports`** if you add a new top-level scope, and confirm `pnpm package` still passes `publint`.

Reviewers check this. A PR that ships a new component without updating its scope `AGENTS.md` is not complete.
